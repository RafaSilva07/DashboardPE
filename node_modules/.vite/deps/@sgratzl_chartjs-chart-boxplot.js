import { J as drawPoint, V as plugin_tooltip, W as registry, X as merge, Y as formatNumber, a as BarElement, i as BarController, l as CategoryScale, m as Element, u as Chart, v as LinearScale } from "./chart-CGw3tHuO.js";
//#region node_modules/@sgratzl/boxplots/build/index.js
/**
* @sgratzl/boxplots
...
function nrd(iqr, variance, len) {
	...
	if (typeof iqr === "number") s = Math.min(s, iqr / 1.34);
	return 1.06 * s * Math.pow(len, -.2);
}
function kde(stats) {
...
function quantilesNearest(arr, length = arr.length) {
	return quantilesInterpolate(arr, length, (i, j, fraction) => fraction < .5 ? i : j);
}
function quantilesMidpoint(arr, length = arr.length) {
	return quantilesInterpolate(arr, length, (i, j) => (i + j) * .5);
}
function quantilesFivenum(arr, length = arr.length) {
	const n = length;
	const n4 = Math.floor((n + 3) / 2) / 2;
	const compute = (d) => .5 * (arr[Math.floor(d) - 1] + arr[Math.ceil(d) - 1]);
	return {
		q1: compute(n4),
...
function computeWhiskers(s, valid, min, max, { eps, quantiles, coef, whiskersMode }) {
	...
	for (let i = 0; i < valid; i += 1) {
		...
		if (v >= whiskerLow || same(v, whiskerLow)) {
			if (whiskersMode === "nearest") whiskerLow = v;
			break;
		}
		if (outlierLow.length === 0 || !same(outlierLow[outlierLow.length - 1], v)) outlierLow.push(v);
	}
	const reversedOutlierHigh = [];
	for (let i = valid - 1; i >= 0; i -= 1) {
		const v = s[i];
		if (v <= whiskerHigh || same(v, whiskerHigh)) {
			if (whiskersMode === "nearest") whiskerHigh = v;
			break;
		}
		if ((reversedOutlierHigh.length === 0 || !same(reversedOutlierHigh[reversedOutlierHigh.length - 1], v)) && (outlierLow.length === 0 || !same(outlierLow[outlierLow.length - 1], v))) reversedOutlierHigh.push(v);
	}
	return {
		median,
		q1,
		q3,
		iqr,
		outlier: outlierLow.concat(reversedOutlierHigh.reverse()),
		whiskerHigh,
		whiskerLow
	};
}
...
//#endregion
//#region node_modules/@sgratzl/chartjs-chart-boxplot/build/index.js
/**
...
function determineStatsOptions(options) {
	return {
		coef: options == null || typeof options.coef !== "number" ? defaultStatsOptions.coef : options.coef,
		quantiles: determineQuantiles(options == null || options.quantiles == null ? quantilesType7 : options.quantiles),
		whiskersMode: options == null || typeof options.whiskersMode !== "string" ? defaultStatsOptions.whiskersMode : options.whiskersMode
	};
}
function boxplotStats(arr, options) {
	const r = boxplot(typeof Float64Array !== "undefined" && !(arr instanceof Float32Array || arr instanceof Float64Array) ? Float64Array.from(arr) : arr, determineStatsOptions(options));
	return {
...
function computeSamples(min, max, points) {
	...
	for (let v = min; v <= max && inc > 0; v += inc) samples.push(v);
	if (samples[samples.length - 1] !== max) samples.push(max);
	return samples;
}
function violinStats(arr, options) {
	if (arr.length === 0) return;
	const stats = boxplot(typeof Float64Array !== "undefined" && !(arr instanceof Float32Array || arr instanceof Float64Array) ? Float64Array.from(arr) : arr, determineStatsOptions(options));
	const coords = computeSamples(stats.min, stats.max, options.points).map((v) => ({
		v,
...
function asViolinStats(value, options) {
	if (!value) return;
	if (typeof value.median === "number" && Array.isArray(value.coords)) return value;
	if (!Array.isArray(value)) return;
...
var StatsBase$1 = class StatsBase extends Element {
	...
	_drawItems(ctx) {
		...
		const { options } = this;
		if (options.itemRadius <= 0 || !props.items || props.items.length <= 0) return;
		ctx.save();
		...
		if (vert) props.items.forEach((v) => {
			if (!outliers.has(v)) drawPoint(ctx, pointOptions, props.x - props.width / 2 + random() * props.width, v);
		});
		else props.items.forEach((v) => {
			if (!outliers.has(v)) drawPoint(ctx, pointOptions, v, props.y - props.height / 2 + random() * props.height);
		});
		ctx.restore();
	}
	_drawOutliers(ctx) {
		...
		const { options } = this;
		if (options.outlierRadius <= 0 || !props.outliers || props.outliers.length === 0) return;
		ctx.save();
		...
		if (vert) props.outliers.forEach((v) => {
			drawPoint(ctx, pointOptions, props.x, v);
		});
		else props.outliers.forEach((v) => {
	...
	_drawMeanDot(ctx) {
		...
		const { options } = this;
		if (options.meanRadius <= 0 || props.mean == null || Number.isNaN(props.mean)) return;
		ctx.save();
		...
		if (vert) drawPoint(ctx, pointOptions, props.x, props.mean);
		else drawPoint(ctx, pointOptions, props.mean, props.y);
		ctx.restore();
	}
	...
	inRange(mouseX, mouseY, useFinalPosition) {
		if (Number.isNaN(this.x) && Number.isNaN(this.y)) return false;
	...
	_outlierIndexInRange(mouseX, mouseY, useFinalPosition) {
		...
		if (vertical && Math.abs(mouseX - props.x) > hitRadius || !vertical && Math.abs(mouseY - props.y) > hitRadius) return null;
		const toCompare = vertical ? mouseY : mouseX;
		for (let i = 0; i < outliers.length; i += 1) if (Math.abs(outliers[i] - toCompare) <= hitRadius) return vertical ? {
			index: i,
			x: props.x,
			y: outliers[i]
		} : {
	...
	_itemIndexInRange(mouseX, mouseY, useFinalPosition) {
		...
		const vert = this.isVertical();
		const { options } = this;
		if (options.itemRadius <= 0 || !props.items || props.items.length <= 0) return null;
		const random = rnd(this._datasetIndex * 1e3 + this._index);
		const outliers = new Set(props.outliers || []);
		if (vert) for (let i = 0; i < props.items.length; i++) {
			const y = props.items[i];
			if (!outliers.has(y)) {
				const x = props.x - props.width / 2 + random() * props.width;
				if (Math.abs(x - mouseX) <= hitRadius && Math.abs(y - mouseY) <= hitRadius) return {
					index: i,
					x,
					y
				};
		...
		else for (let i = 0; i < props.items.length; i++) {
	...
	_getOutliers(useFinalPosition) {
		return this.getProps(["outliers"], useFinalPosition).outliers || [];
	}
	tooltipPosition(eventPosition, tooltip) {
		if (!eventPosition || typeof eventPosition === "boolean") return this.getCenterPoint();
		if (tooltip) {
...
var BoxAndWiskers = class extends StatsBase$1 {
	...
	_drawBoxPlot(ctx) {
		if (this.isVertical()) this._drawBoxPlotVertical(ctx);
		else this._drawBoxPlotHorizontal(ctx);
	}
	_drawBoxPlotVertical(ctx) {
		...
		const { width } = props;
		const x0 = x - width / 2;
		if (props.q3 > props.q1) ctx.fillRect(x0, props.q1, width, props.q3 - props.q1);
		else ctx.fillRect(x0, props.q3, width, props.q1 - props.q3);
		ctx.save();
		if (options.medianColor && options.medianColor !== "transparent" && options.medianColor !== "#0000") ctx.strokeStyle = options.medianColor;
		ctx.beginPath();
		...
		if (options.lowerBackgroundColor && options.lowerBackgroundColor !== "transparent" && options.lowerBackgroundColor !== "#0000") {
			...
			if (props.q3 > props.q1) ctx.fillRect(x0, props.median, width, props.q3 - props.median);
			else ctx.fillRect(x0, props.median, width, props.q1 - props.median);
		}
		ctx.restore();
		if (props.q3 > props.q1) ctx.strokeRect(x0, props.q1, width, props.q3 - props.q1);
		else ctx.strokeRect(x0, props.q3, width, props.q1 - props.q3);
		ctx.beginPath();
	...
	_drawBoxPlotHorizontal(ctx) {
		...
		const { height } = props;
		const y0 = y - height / 2;
		if (props.q3 > props.q1) ctx.fillRect(props.q1, y0, props.q3 - props.q1, height);
		else ctx.fillRect(props.q3, y0, props.q1 - props.q3, height);
		ctx.save();
		if (options.medianColor && options.medianColor !== "transparent") ctx.strokeStyle = options.medianColor;
		ctx.beginPath();
		...
		if (options.lowerBackgroundColor && options.lowerBackgroundColor !== "transparent") {
			ctx.fillStyle = options.lowerBackgroundColor;
			if (props.q3 > props.q1) ctx.fillRect(props.median, y0, props.q3 - props.median, height);
			else ctx.fillRect(props.median, y0, props.q1 - props.median, height);
		}
		ctx.restore();
		if (props.q3 > props.q1) ctx.strokeRect(props.q1, y0, props.q3 - props.q1, height);
		else ctx.strokeRect(props.q3, y0, props.q1 - props.q3, height);
		ctx.beginPath();
...
var Violin = class extends StatsBase$1 {
	draw(ctx) {
		...
		if (props.median != null) drawPoint(ctx, {
			pointStyle: "rectRot",
			radius: 5,
			borderWidth: this.options.borderWidth
		}, props.x, props.y);
		if (props.coords && props.coords.length > 0) this._drawCoords(ctx, props);
		this._drawOutliers(ctx);
		this._drawMeanDot(ctx);
		ctx.restore();
		this._drawItems(ctx);
	...
	_drawCoords(ctx, props) {
		let maxEstimate;
		if (props.maxEstimate == null) maxEstimate = props.coords.reduce((a, d) => Math.max(a, d.estimate), Number.NEGATIVE_INFINITY);
		else maxEstimate = props.maxEstimate;
		ctx.beginPath();
		if (this.isVertical()) {
			const { x, width } = props;
...
var interpolators = { number(from, to, factor) {
...
function interpolateNumberArray(from, to, factor) {
	if (typeof from === "number" && typeof to === "number") return interpolators.number(from, to, factor);
	if (Array.isArray(from) && Array.isArray(to)) return to.map((t, i) => interpolators.number(from[i], t, factor));
	return to;
}
function interpolateKdeCoords(from, to, factor) {
	if (Array.isArray(from) && Array.isArray(to)) return to.map((t, i) => ({
...
function patchInHoveredOutlier(item) {
	...
	const that = this;
	if (value && that._tooltipOutlier != null && item.datasetIndex === that._tooltipOutlier.datasetIndex) value.hoveredOutlierIndex = that._tooltipOutlier.index;
	if (value && that._tooltipItem != null && item.datasetIndex === that._tooltipItem.datasetIndex) value.hoveredItemIndex = that._tooltipItem.index;
}
...
function baseDefaults(keys) {
	...
	return {
		...
		transitions: {
			show: { animations: { colors: {
				type: "color",
				properties: colorKeys,
				from: "transparent"
			} } },
			hide: { animations: { colors: {
				type: "color",
				properties: colorKeys,
				to: "transparent"
			} } }
...
function defaultOverrides() {
	return { plugins: { tooltip: {
		...
		callbacks: { beforeLabel: patchInHoveredOutlier }
	} } };
}
var StatsBase = class extends BarController {
	_transformStats(target, source, mapper) {
		for (const key of [
			...
			const v = source[key];
			if (typeof v === "number") target[key] = mapper(v);
		}
		for (const key of ["outliers", "items"]) if (Array.isArray(source[key])) target[key] = source[key].map(mapper);
	}
	...
	getLabelAndValue(index) {
		...
		if (!vScale || !parsed || r.value === "NaN") return r;
		r.value = {
			raw: parsed,
		...
		r.value.toString = function toString() {
			if (this.hoveredOutlierIndex >= 0) return `(outlier: ${this.outliers[this.hoveredOutlierIndex]})`;
			if (this.hoveredItemIndex >= 0) return `(item: ${this.items[this.hoveredItemIndex]})`;
			return s;
		};
		return r;
	}
...
function patchController(type, config, controller, elements = [], scales = []) {
	...
	if (Array.isArray(elements)) registry.addElements(...elements);
	else registry.addElements(elements);
	if (Array.isArray(scales)) registry.addScales(...scales);
	else registry.addScales(scales);
	const c = config;
	c.type = type;
	return c;
}
var BoxPlotController = class extends StatsBase {
	...
	_transformStats(target, source, mapper) {
		...
		for (const key of ["whiskerMin", "whiskerMax"]) target[key] = mapper(source[key]);
	}
};
...
BoxPlotController.defaults = merge({}, [
	...
	baseDefaults(boxOptionsKeys),
	{
		animations: { numbers: {
			type: "number",
			properties: BarController.defaults.animations.numbers.properties.concat([
				"q1",
				"q3",
				"min",
				"max",
...
var ViolinController = class extends StatsBase {
	...
	_transformStats(target, source, mapper) {
		...
		if (Array.isArray(source.coords)) target.coords = source.coords.map((c) => ({
			...c,
			v: mapper(c.v)
		}));
	}
